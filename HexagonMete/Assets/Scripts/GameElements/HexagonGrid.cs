...
public class HexagonGrid : CustomBehaviour
{
    ...
    private void CreateGrid()
    {
        ...
        for (int y = 0; y < gridSize.y; y++)
        {
            for (int x = 0; x < gridSize.x; x++)
            {
                ...
                mHexagons[x, y].Init(GameManager);
                mHexagons[x, y].SetHexagon(new Vector2Int(x, y), position, HexagonType.NORMAL, true);
            }
        }
    }
    ...
    private void CreateTripleHexagons()
    {
        if (mHexagons == null)
        {
            Debug.Log("mHexagons is null. Before this, you need to call CreateGrid");
            return;
        }
            ...
            for (int x = 0; x < mHexagons.GetLength(0); x++)
            {
                ...
                if (IsValidIndex(upHexCoor))
                {
                    ...
                    if (IsValidIndex(UpLeftHexCoor))
                    {
                        ...
                        mTripleHexagons[index].Init(GameManager);
                        mTripleHexagons[index].SetTripleHexagon(index, currentHex, upHex, upLeftHex, false);
                    ...
                    if (IsValidIndex(UpRightHexCoor))
                    {
                        ...
                        mTripleHexagons[index] = Instantiate(prefabTripleHexagon, transformTripleHexagonsParent);
                        mTripleHexagons[index].Init(GameManager);
                        mTripleHexagons[index].SetTripleHexagon(index, GetHexagon(mHexagons[x, y].coordinate), upHex, upRightHex, true);
    ...
    public IEnumerator IAllHexagonFallAnim()
    {
        while (true)
        {
            ...
            // This loop make access array elements like f(x) = -x. You go diagonally from the bottom left to the top right corner in the array
            // For [3,3] a array, access order
            // [3][6][8]
            // [1][4][7]
            // [0][2][5]

            //For falling any hexagon in grid
            for (int a = 0; a < max; a++) 
            {
                for (int x = 0, y = a; x < mHexagons.GetLength(0); x++, y = a - x)
                {
    ...
    /// <summary>
    /// This function controls TripleHexagons colors and groups thier hexagons of the same color by their neighborhood
    /// </summary>
    ...
    /// <summary>
    /// Return, does coordinate stay out of index in grid?
    /// </summary>
    ...
    /// <summary>
    /// Returns TripleHexagons list that the hexagon is in
    /// </summary>
    ...
    /// <summary>
    /// Return coordinate of lowest null in one cloum
    /// </summary>
